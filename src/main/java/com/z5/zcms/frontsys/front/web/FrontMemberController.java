...
public class FrontMemberController {
	...
	public String call(@ModelAttribute("frontBoardVo") FrontBoardVo frontBoardVo, @ModelAttribute("zUserVo") ZUserVo zUserVo, HttpServletRequest request, Model model,
		...
		try {
			...
			// 신고 작성 페이지 입력 값 넣는 
			zUserVo.setUsername(name);							// 사용자 성명 *
		...
		model.addAttribute("name", name);			// 1.신고자 성명
		model.addAttribute("attachFile", attachFile);		// 2.첨부파일
		model.addAttribute("title", title);			// 3.신고제목
		model.addAttribute("content", content);		// 4.신고내용
		model.addAttribute("email", email);			// 5.메일
		model.addAttribute("date", date); 			// 6.발생일시
		model.addAttribute("rmenuno", menuno); 		// 7.menuno
		model.addAttribute("rcateno", cateno); 		// 8.cateno
		model.addAttribute("mobileno", mobileno); 	// 9.휴대폰
		model.addAttribute("sDupInfo", sDupInfo);   // 10.DI
